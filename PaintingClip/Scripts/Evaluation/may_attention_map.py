...
for tok, g in zip(tokens, token_grads):
    if tok in ("<|startoftext|>", "<|endoftext|>"):           continue
    if tok.endswith("</w>"):     
        tok = tok.replace("</w>", "")
        word += tok; agg[word] = agg.get(word, 0) + g; word = ""
    else:
        word += tok
if word: agg[word] = agg.get(word, 0)
...
grads_img = np.log1p(grads_img);  grads_img -= grads_img.min(); grads_img /= grads_img.max()
grads_img[grads_img < threshold] = 0

N          = grads_img.size
gsz        = int(np.sqrt(N))
if gsz**2 != N: raise ValueError("Gradient grid is not square.")
grads_img  = gaussian_filter(zoom(grads_img.reshape(gsz, gsz), zoom=32), sigma=2)
grads_img -= grads_img.min(); grads_img /= grads_img.max()
...
plt.axis("off"); plt.title("Attention heat-map overlay")

base   = image_path.rsplit("/",1)[-1].split(".")[0]
stamp  = time.strftime("%H-%M-%S")
outimg = f"{base}_{stamp}_overlay.png"
plt.savefig(outimg, bbox_inches="tight"); plt.close()

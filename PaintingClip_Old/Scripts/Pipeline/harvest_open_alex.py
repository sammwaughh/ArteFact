...
def create_excel_file(painter_name, works):
    ...
    wb.save(filename)
    excel_logger.info(
        f"Excel file '{filename}' created with sheets: Main, Filtered, and Downloadable."
    )
    return filename
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    download_logger.info(
                        f"Row {index}: Successfully downloaded as: {filepath} in {elapsed_sec} seconds"
                    )
                    ...
                    download_logger.warning(
                        f"Row {index}: Failed with status {response.status_code} on link: {link}"
                    )

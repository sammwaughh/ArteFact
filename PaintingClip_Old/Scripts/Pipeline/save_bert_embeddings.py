...
def find_matching_paintings(paintings_range, painter_name):
    ...
    if mask_first.sum() > 0:
        print(
            f"No exact match found. Found {mask_first.sum()} paintings with first name '{first_name}'."
        )
    ...
    if mask_last.sum() > 0:
        print(
            f"No exact or first name match found. Found {mask_last.sum()} paintings with last name '{last_name}'."
        )
...
def main():
    ...
    for idx, row in painters_subset.iterrows():
        ...
        if os.path.exists(cache_file):
            print(
                f"Cache for artist {artist} ({artist_dir_name}) already exists. Skipping."
            )
            continue
        print(
    ...
    print(
        f"Processed {total_processed} painters in {elapsed_rounded} seconds. Rate: {rate} seconds per painter."
    )

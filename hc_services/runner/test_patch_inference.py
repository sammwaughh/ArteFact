...
def main():
    ...
    whole_image_results = run_inference(
        ...
        cell=None,  # None means whole image
        top_k=TOP_K
    )
    ...
    if whole_image_results:
        ...
        for region_name, (row, col) in test_cells:
            ...
            if region_score:
                print(f"  {region_name} ({row},{col}): score={region_score:.4f}, rank={region_rank}")
            else:
    ...
    if save_results:
        results_data = {
            ...
            "corners": {
                name: {
                    "position": list(pos),
                    "results": run_inference(IMAGE_PATH, cell=pos, grid_size=GRID_SIZE, top_k=5)
                }

...
class GradECLIP:
        ...
        if DEBUG:
            ...
            if data is not None:
                if isinstance(data, torch.Tensor):
                    print(f"{indent}  Shape: {data.shape}, Device: {data.device}, Dtype: {data.dtype}")
                    # Only compute statistics for floating point tensors
                    if data.dtype in [torch.float16, torch.float32, torch.float64]:
                        print(f"{indent}  Range: [{data.min().item():.4f}, {data.max().item():.4f}]")
                        print(f"{indent}  Mean: {data.mean().item():.4f}, Std: {data.std().item():.4f}")
                    else:
                        # For integer tensors, just show range
                        print(f"{indent}  Range: [{data.min().item()}, {data.max().item()}]")
    ...
    def _forward_with_hooks(
        ...
        self.debug_print(f"Using layer {layer_idx} of {len(vision_layers)} total layers", level=1)
        ...
        def forward_hook_layer(module, input, output):
            ...
            if len(input) > 0:
                ...
                x = module.self_attn.layer_norm(hidden_states) if hasattr(module.self_attn, 'layer_norm') else hidden_states
                ...
                if hasattr(module.self_attn, 'q_proj'):
                    ...
                    captures['Q'] = Q
                    captures['K'] = K
                    captures['attn_weights'] = attn_weights.mean(dim=1)  # Average over heads
                    captures['attn_output'] = attn_output
                    
                    self.debug_print(f"Attention weights shape: {attn_weights.shape}", level=3)
                    self.debug_print(f"Attention output shape: {attn_output.shape}", level=3)
            
            # Store the layer output
            hidden_states_out = output[0]
            captures['hidden_states'] = hidden_states_out
            self.debug_print("Output hidden states captured", hidden_states_out, level=3)
    ...
    def compute_gradeclip_map(
        ...
        self.debug_print(f"Layer index: {layer_idx}, Use K-similarity: {use_k_similarity}")
            ...
            with torch.set_grad_enabled(True):
                ...
                if attn_weights is not None:
                    ...
                    if 'hidden_states_pre' in captures:
                        ...
                        cls_attn = torch.softmax(cls_attn * 5.0, dim=-1)  # Temperature scaling
                        self.debug_print("Using cosine similarity attention (fallback)", cls_attn)
                    else:
                ...
                attn_patches = cls_attn[1:num_patches+1]  # Attention from CLS to patches
                ...
                importance = (grad_cls * V_patches).sum(dim=-1)  # Element-wise multiply and sum over channels
                importance = importance * attn_patches  # Weight by attention
                importance = torch.relu(importance)  # ReLU activation
                
                self.debug_print("Importance (before ReLU)", (grad_cls * V_patches).sum(dim=-1))
...
def main():
    ...
    saliency = gradeclip.compute_gradeclip_map(
        ...
        use_k_similarity=USE_K_SIMILARITY
    )
